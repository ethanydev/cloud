...
exports.startReservation = async (req, res) => {
  try {
    ...
    if (reservation.user_id.toString() !== req.user.id) {
      return res.status(403).json({ message: "본인 예약만 시작할 수 있습니다." });
    }
...
exports.endReservation = async (req, res) => {
  try {
    ...
    const isOwner = reservation.user_id.toString() === req.user.id;
    const isAdmin = req.user.role === "admin";

    if (!isOwner && !isAdmin) {
      return res.status(403).json({ message: "본인 예약 또는 관리자만 종료할 수 있습니다." });
    }
